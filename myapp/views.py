...
def result(request):
    ...
    '''
                ...
                database_name1 = "url_history1"
                my_database1 = client.create_database(database_name1)         
                new_document1 = my_database1.create_document(json_document)      

                ## page rank
                import json
                from urllib.parse import urlparse
                try:
                    index_database_name = "mud_page_rank"
                    index_database = client.create_database(index_database_name)
                    parsed_uri = urlparse(text)
                    _domain = '{uri.netloc}'.format(uri=parsed_uri)
                    doc_exists = _domain in index_database
                    if not doc_exists:
                        index_json_document = {
                            "_id":_domain, 
                            "domain": _domain,
                        ...
                        index_document = my_database.create_document(index_json_document)
                        index_database = client.create_database(index_database_name)         
                        write_index = index_database.create_document(index_document) 
                    else:
                        _old_doc = index_database[_domain]
                        _old_doc["count"]+=1
                        _old_doc.save() 
...
def fetchanalysis(request):
    try:
        ...
        #figure(num=None, figsize=(12,14), dpi=40, facecolor='w', edgecolor='k')
        ...
        fig.savefig(location2, dpi=40,bbox_inches='tight')
        ...
        fig.savefig(location3, dpi=40,bbox_inches='tight')
        ...
        fig.savefig(location4, dpi=40,bbox_inches='tight')
        ...
        fig.savefig(location5, dpi=40,bbox_inches='tight')
        ...
        fig.savefig(location8, dpi=40,bbox_inches='tight')
        ...
        fig.savefig(location6, dpi=40,bbox_inches='tight')
        from collections import Counter
        hours=[]
        for i in df['Time']:
            if i[11:13] != "":
        ...
        fig.savefig(location7, dpi=40,bbox_inches='tight')
        ...
        fig.savefig(location9, dpi=40,bbox_inches='tight')
